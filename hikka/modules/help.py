...
class Help(loader.Module):
    ...
    def __init__(self):
        self.config = loader.ModuleConfig(
            loader.ConfigValue(
                "core_emoji",
                "<emoji document_id=5341492148468465410>📂</emoji>",
            ...
            loader.ConfigValue(
                "desc_icon",
                "<emoji document_id=5258023599419171861>🔧</emoji>",
                lambda: "Desc emoji",
            ),
        )
    ...
    async def help(self, message: Message):
        ...
        for mod in self.allmodules.modules:
            ...
            for cmd in icommands:
                if first:
                    tmp += f": ( <emoji document_id=5870531058755178453>🤖</emoji> {cmd}"
                    first = False
                else:
                    tmp += f" | <emoji document_id=5870531058755178453>🤖</emoji> {cmd}"
        ...
        await utils.answer(
            ...
            (
                ...
                + html_decoration.blockquote("<emoji document_id=4913497231492908158>👤</emoji> User modules:\n")
                + html_decoration.blockquote("{}")
                + "{}"
            ).format(
                reply,
